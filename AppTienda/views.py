...
def proveedor(request):
    if request.method == "POST":
        ...
        if form_proveedor.is_valid():
            ...
            proveedor_nuevo = Proveedor(razon_social=info_proveed["razon_social"], cuit=info_proveed["cuit"],
                                        telefono=info_proveed["telefono"], tipo_prenda=info_proveed["tipo_prenda"])
...
def prendas(request):
    if request.method == "POST":
        ...
        if form_prendas.is_valid():
            ...
            prendas_nuevo = Prendas(tipo_prenda=info_prendas["tipo_prenda"], talle=info_prendas["talle"],
                                    color=info_prendas["color"])
            prendas_nuevo.save()
...
def cliente(request):
    if request.method == "POST":
        ...
        if form_cliente.is_valid():
            ...
            cliente_nuevo = Cliente(nombre=info["nombre"], apellido=info["apellido"], email=info["email"],
...
def buscar(request):
    if "apellido" in request.GET:
        ...
        cliente_busqueda = Cliente.objects.filter(apellido=apellido)
        if cliente_busqueda:
            return render(request, 'resultado_busqueda.html', {"apellido_cliente": cliente_busqueda})
        else:
            return render(request, 'busqueda_cliente.html', {"mensaje": "Ingrese apellido del cliente a buscar"})
    else:
        return render(request, 'busqueda_cliente.html', {"mensaje": "Ingrese apellido del cliente a buscar"})
